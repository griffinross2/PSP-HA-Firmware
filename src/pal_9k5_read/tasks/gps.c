...
/* API FUNCTIONS */
/*****************/
Status gps_init() { return STATUS_OK; }

void task_gps(TaskHandle_t* handle_ptr) {
    if (max_m10s_init(&s_gps_conf) != STATUS_OK) {
        PAL_LOGE("Failed to init GPS\n");
        gpio_write(PIN_GPS_RST, GPIO_LOW);
    ...
    while (1) {
        GPS_Fix_TypeDef fix;
        Status ret = max_m10s_poll_fix(&s_gps_conf, &fix);
        if (ret == STATUS_OK || ret == STATUS_BUSY) {
#ifdef HWIL_TEST
